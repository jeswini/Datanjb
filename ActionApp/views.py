...
def actionApi(request):
    if request.method == 'POST':
        ...
        if actionName == "Decision Tree":
            ...
            json_object = json.loads(jsonData)
            dataFile = pd.json_normalize(json_object)
            x_columns = ["age", "workclass", "fnlwgt", "education", "education-num", "marital-status", "occupation",
        ...
        if actionName == "UPPERCASE":
            ...
            for column_name in sortColumnNames:
                if dataFile[column_name].dtype == 'O':
                    dataFile[column_name] = dataFile[column_name].str.upper()
            result = dataFile.to_json(orient="records")
            message = json.loads(result)
        ...
        if actionName == "lowercase":
            ...
            for column_name in sortColumnNames:
                if dataFile[column_name].dtype == 'O':
                    dataFile[column_name] = dataFile[column_name].str.lower()
            result = dataFile.to_json(orient="records")
            message = json.loads(result)
        ...
        #     dataFile = pd.json_normalize(json_object)
        #
        #     # Extract file path from input parameter
        #     fileNamePath = data["fileList"][0]
        #
        #     # Read CSV file into DataFrame
        #     dataFile = pd.read_csv(fileNamePath)
        #
        #     # Convert object columns to uppercase
        #     object_cols = dataFile.select_dtypes(include=['object']).columns
        #     dataFile[object_cols] = dataFile[object_cols].apply(lambda x: x.str.upper())
        #
        #     # Convert DataFrame to JSON and return
        #     result = dataFile.to_json(orient="records")
        #     message = json.loads(result)
        # else:
        #     message = {"error": "Invalid action name"}
